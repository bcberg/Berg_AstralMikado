function p = estPercProb_exp(numAsters,l,D,astralNum,Nsamp,useMEX)
%ESTPERCPROB_EXP uses Monte Carlo sampling to estimate percolation
%probabilities of astral networks; filaments have exponentially distributed
%lengths w/ mean l
%   Inputs:
...
if useMEX && numFil < 1e4
    for idx = 1:Nsamp
        [network,crossings,~] = generateAstralNetwork_exp_mex(numAsters,l,D,...
            astralNum, true);
    ...
    for idx = 1:Nsamp
        [network,crossings,~] = generateAstralNetwork_exp(numAsters,l,D, ...
            astralNum, true);
        [percTF,~] = percCheck(crossings,network.nodes,D);
        percCount = percCount + percTF;
    end
