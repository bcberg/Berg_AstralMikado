function curves = getAllPercCurves_exp(l,D,astralNumList,densSpec, ...
    Nsamp,Ncores,saveDirectory)
%GETALLPERCCURVES_EXP Computes percolation probability curves for a 
% list of astral network types; filaments have exponentially distributed 
% lengths w/ mean l
...
addAttachedFiles(pool,{'generateAstralNetwork_exp.m','percCheck.m', ...
    'estPercProb_exp.m'})
F = parallel.FevalFuture.empty(Njobs,0);
job_idx = 1;
for idx = 1:numNetTypes
    for jdx = 1:numUniqueDensVals_byrow(idx)
        F(job_idx) = parfeval(pool,@estPercProb_exp,1, ...
