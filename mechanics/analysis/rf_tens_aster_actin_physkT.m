...
nameprefix = 'rf_tens_aster_actin_physkT251114';
...
if newReports
    filtering.TF = true;
    filtering.catchDrops = true;   % at higher k_B*T, lowest applied force
    % does not always significantly deform the network
    dir = "/home/bcberg/Documents/AstralMikadoCYM/runs/" + ...
        "rf_tens_aster_actin_physkT/";

    % ensure the following parameters match those used in 
    % rf_tens_aster_actin_physkT.cym.tpl and its driver (for numRep)
    dens = 75;
    len_fil = 0.1;
    ...
    for idx = 1:numel(tensModuli)
        ...
        [thisModulus, linefit, gof] = getModulus(thisInitCoM,thisFinalCoM,...
            forceVals,"extension",filtering);
        tensModuli(netIdx,repIdx) = thisModulus;
    ...
    save(fullfile(saveDir,dataSubdir,matFile),'D','len_fil', ...
        'nFilPerAsterList','tensModuli','lineFits','fitStats','numRep',...
...
if refit
    ...
    save(fullfile(saveDir,dataSubdir,matFile),'D','len_fil', ...
        ...
        'tensModuli','lineFits','fitStats','percStatus','filtering')
end
...
ylabel("2D Young's modulus [pN/{\mu}m]")
% ylim([0, inf])
...
for netIdx = 1:nNetTypes
    ...
    for repIdx = 1:numRep
        ...
        if tensModuli(netIdx,repIdx) == 0
            ...
            l3 = plot(forceVals,forceVals / tensModuli(netIdx,repIdx),'--k', ...
    ...
    xlabel(dispPlots,'Tensile force [pN]')
    ylabel(dispPlots,'y-Displacement [{\mu}m]')
