...
nameprefix = 'rep_tens241116';
newReports = false;
if newReports
    dir = "/home/bcberg/Documents/AstralMikadoCYM/runs/rep_full_tens_aster/";
...
for netIdx = 1:nNetTypes
    ...
    for repIdx = 1:numRep
        ...
        hold on
        plot(forceVals, disps.(networkLabel)(:,2,repIdx), '*-b')
        plot(forceVals, forceVals / tensModuli(netIdx,repIdx), '-k')
        hold off
        xlim('tight')
        % yl = ylim;
        % ylim([0,yl(2)])
        xlabel('Tensile force [pN]')
        ylabel('y-Displacement [{\mu}m]')
        tileIdx = tileIdx + 1;
    end
