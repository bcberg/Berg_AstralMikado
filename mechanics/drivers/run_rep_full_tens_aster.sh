...
mkdir -p rep_full_tens_aster
cd rep_full_tens_aster
# clean any files/folders created from previous runs
rm -rf *
# create config files in $rundir/rep_full_tens_aster for every force value specified in template
$cymroot/python/run/preconfig.py $numRep $myroot/templates/rep_full_tens_aster.cym.tpl
# place them into their own subdirectories of rep_full_tens_aster
$cymroot/python/look/collect.py run%04i/config.cym rep_full_tens_aster????.cym
