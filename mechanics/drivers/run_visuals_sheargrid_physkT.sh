...
export runName="visuals_sheargrid_physkT"
...
rm -rf *
# create config files in $rundir/shear_visualgrid for every force value specified in template
$cymroot/python/run/preconfig.py $myroot/templates/${runName}.cym.tpl
# place them into their own subdirectories of shear_visualgrid
...
do
    runCode=$(printf "%04d" "$runIdx")
    cp $myroot/displays/visuals_sheargrid.cyp run${runCode}/display.cyp
done
