...
mkdir -p modulus_distr_tens
cd modulus_distr_tens
# clean any files/folders created from previous runs
rm -rf *
# create config files in $rundir/modulus_distr_tens for every force value specified in template
$cymroot/python/run/preconfig.py $numRep $myroot/templates/modulus_distr_tens.cym.tpl
# place them into their own subdirectories of modulus_distr_tens
$cymroot/python/look/collect.py run%04i/config.cym modulus_distr_tens????.cym
