% Inspect Cytosim network generation via percolation probabilities
% Brady Berg, 2025-12-03

% 4 parameter sets: (viscosity, kT, bind_rate, r_bind, crslnk_per_fil, crslnk_period)
% (standard cold params), (standard physiological params), (viscous set 1), (viscous set 2)
[[paramVec = [(0.5, 2.1e-5, 1, 1e-3, 30, 50), (0.5, 4.2e-3, 1, 1e-3, 30, 50), (1e3, 2.1e-5, 1e2, 1e-4, 30, 5), (1e3, 2.1e-5, 1e2, 2.5e-5, 120, 5)] ]]

set simul system
{
    time_step = 0.1
    viscosity = [[paramVec[0]]]
    kT = [[paramVec[1]]]
    verbose = 0
...
% initialize asters
% density sweep values duplicated from Matlab sweep at astral number 2 (35 values)
[[dens = [10.0,11.4,12.8,14.6,16.6, 18.8,21.4,24.2,27.6,31.4,35.6,40.4,45.8,52.0,59.0, 67.0,76.0,86.4,98.0,111.4,126.4,143.4,162.8,184.8,209.8, 238.2,270.4,307.0,348.6,395.6,449.2,510.0,578.8,657.2,746.0]]]
...
set hand binder
{
	binding_rate = [[paramVec[2]]]
	binding_range = [[paramVec[3]]]
	unbinding_rate = 0
...
new [[paramVec[4]*nFil]] crosslinker {attach1 = actin}
...
run system
{
    nb_steps = [[paramVec[5]*10]]
    nb_frames = [[paramVec[5]]]
}
...
% KEY READOUT: report states of crosslinkers to determine network connectivity
